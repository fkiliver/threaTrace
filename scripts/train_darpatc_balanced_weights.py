...
class SAGENet(torch.nn.Module):
	...
	def forward(self, x, data_flow):
		...
		return x  # 返回原始 logits，不使用 log_softmax，因为 CrossEntropyLoss 内部会处理
...
def test(mask):
	...
	for data_flow in loader(mask):
		out = model(data.x.to(device), data_flow.to(device))
		pred = out.max(1)[1]
		pro  = F.softmax(out, dim=1)  # 对 logits 应用 softmax
		pro1 = pro.max(1)
...
def train_pro():
	...
	global model, loader, optimizer, device, fp, tn, loop_num, criterion
	...
	show('Computed class weights:', class_weights.cpu().numpy())
